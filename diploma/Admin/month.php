...
$sql = "SELECT * FROM vechicle WHERE `date_time` >= DATE_SUB( CURDATE( ) ,INTERVAL 1 MONTH ) ORDER BY id";
...
<body>
    ...
    $result1 = $mysqli->query("SELECT sum(tax) FROM vechicle  WHERE `date_time` >=  DATE_SUB( CURDATE( ),INTERVAL 1 MONTH)") or die(mysqli_error());
    ...
    <?php } ?>
      ...
      <a onclick="javascript:window.history.back(-2);return false;">
