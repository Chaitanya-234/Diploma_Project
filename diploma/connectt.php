...
<?php
	...
	if ($conn->connect_error && $connect->connect_error) {
		echo "$connect->connect_error";
		die('connectio failed ...'.$conn->connect_error);
	}
	else{
		$s="SELECT * FROM fastag WHERE VecNo= '".$_POST['vechicleNo']."'";


	$result = $connect->query($s) or die($connect->error);
	...
	$a=$rows['Balance'];
		...
		$execval = $stmt->execute();
		  $sql = "UPDATE fastag SET Balance=Balance -".$_POST['tax']." WHERE VecNo ='".$_POST['vechicleNo']."'";
          $stmtt=$sql or die (mysqli_error());
          $connect->query($stmtt);
		echo $execval;
