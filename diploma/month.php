...
$sql = "SELECT * FROM vechicle WHERE `date_time` >= DATE_SUB( CURDATE( ) ,INTERVAL 1 MONTH ) AND TaxReceiver LIKE "."'".$_SESSION['user']."'ORDER BY id";
...
<body>
    ...
    $result1 = $mysqli->query("SELECT sum(tax) FROM vechicle  WHERE `date_time` >=  DATE_SUB( CURDATE( ),INTERVAL 1 MONTH) AND TaxReceiver LIKE "."'".$_SESSION['user']."'") or die(mysqli_error());
    ...
    <?php } ?>
      ...
      <a href="#" onclick="javascript:window.history.back(-2);return false;">
          <input type="button" class="back" value="Back" name="">
        </a>
