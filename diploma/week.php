...
$sql = "SELECT * FROM vechicle WHERE `date_time` >= DATE_SUB( CURDATE( ) ,INTERVAL 7 DAY ) AND TaxReceiver LIKE "."'".$_SESSION['user']."' ORDER BY id";
...
<body>
    ...
    $result1 = $mysqli->query("SELECT sum(tax) FROM vechicle  WHERE `date_time` >= DATE_SUB( CURDATE( ) ,INTERVAL 7 DAY ) AND TaxReceiver LIKE "."'".$_SESSION['user']."'") or die(mysqli_error());
